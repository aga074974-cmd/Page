...
android {
    ...
    // یک APK جداگانه به ازای هر معماری — و هم‌زمان تعیین‌کنندهٔ ABIهای پشتیبانی‌شده.
    //
    // توجه: در کنار این بلوک نباید `defaultConfig { ndk { abiFilters … } }` وجود داشته
    // باشد؛ AGP این دو را متناقض می‌داند و بیلد را رد می‌کند. خودِ `include` زیر هم نقش
    // فیلتر ABI را بازی می‌کند، پس abiFilters اضافی است.
    //
